use crate::{
	basic::{LogicalType, Repetition, Type as PhysicalType}, errors::ParquetError, file::reader::{FileReader, ParquetReader, SerializedFileReader}, schema::{
		printer::{print_file_metadata, print_parquet_metadata, print_schema}, types::{BasicTypeInfo, SchemaDescPtr, SchemaDescriptor, Type}
	}
};
use super::triplet::TypedTripletIter;
use crate::data_type::{BoolType,Int32Type,Int64Type,Int96Type,FloatType,DoubleType,ByteArrayType,FixedLenByteArrayType};
...
use super::schemas::{ValueSchema,BoolSchema,U8Schema,I8Schema,U16Schema,I16Schema,U32Schema,I32Schema,U64Schema,I64Schema,F32Schema,F64Schema,VecSchema,ArraySchema,StringSchema,TimestampSchema,RootSchema,GroupSchema,TupleSchema,OptionSchema,ListSchema,MapSchema};
use super::reader::{ValueReader,BoolReader,I32Reader,I64Reader,I96Reader,F32Reader,F64Reader,ByteArrayReader,FixedLenByteArrayReader,OptionReader,RepeatedReader,KeyValueReader,RootReader,GroupReader,TupleReader,Reader,MapReader,TryIntoReader};
use std::collections::HashMap;
use std::hash::{Hash,Hasher};
use std::{rc::Rc,fmt,str};
use std::fmt::Debug;
...
impl Deserialize for bool {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		BoolReader{column: TypedTripletIter::<BoolType>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}
	}
}
impl Deserialize for f32 {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		F32Reader{column: TypedTripletIter::<FloatType>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}
	}
}
impl Deserialize for f64 {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		F64Reader{column: TypedTripletIter::<DoubleType>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}
	}
}
impl Deserialize for i8 {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		TryIntoReader(I32Reader{column: TypedTripletIter::<Int32Type>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}, PhantomData)
	}
}
...
impl Deserialize for i32 {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		I32Reader{column: TypedTripletIter::<Int32Type>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}
	}
}
impl Deserialize for u32 {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		MapReader(I32Reader{column: TypedTripletIter::<Int32Type>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}, (|x|Ok(x as u32)) as fn(_)->_)
	}
}
impl Deserialize for i64 {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		I64Reader{column: TypedTripletIter::<Int64Type>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}
	}
}
impl Deserialize for u64 {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		MapReader(I64Reader{column: TypedTripletIter::<Int64Type>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}, (|x|Ok(x as u64)) as fn(_)->_)
	}
}
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		match schema {
			TimestampSchema::Int96 => {
				sum::Sum3::A(MapReader(I96Reader{column: TypedTripletIter::<Int96Type>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}, (|x|Ok(Timestamp(x))) as fn(_)->_))
			}
			TimestampSchema::Millis => {
				sum::Sum3::B(MapReader(I64Reader{column: TypedTripletIter::<Int64Type>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}, (|millis| {
			...
			TimestampSchema::Micros => {
				sum::Sum3::C(MapReader(I64Reader{column: TypedTripletIter::<Int64Type>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}, (|micros| {
...
impl Deserialize for Vec<u8> {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		ByteArrayReader{column: TypedTripletIter::<ByteArrayType>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}
	}
}
impl Deserialize for String {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		MapReader(ByteArrayReader{column: TypedTripletIter::<ByteArrayType>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}, (|x|String::from_utf8(x).map_err(|err:FromUtf8Error|ParquetError::General(err.to_string()))) as fn(_)->_)
	}
}
impl Deserialize for [u8; 1024] {
	...
	fn reader(schema: &Self::Schema, mut path: &mut Vec<String>, curr_def_level: i16, curr_rep_level: i16, paths: &mut HashMap<ColumnPath, (ColumnDescPtr,ColumnReader)>) -> Self::Reader {
		...
		let (col_descr, col_reader) = paths.remove(&col_path).unwrap();
		assert_eq!((curr_def_level, curr_rep_level), (col_descr.max_def_level(), col_descr.max_rep_level()));
		MapReader(FixedLenByteArrayReader{column: TypedTripletIter::<FixedLenByteArrayType>::new(curr_def_level, curr_rep_level, DEFAULT_BATCH_SIZE, col_reader)}, (|bytes: Vec<_>| {
